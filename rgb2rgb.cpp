...
char* rgb2rgb::LoadShader( std::string filename )
{
	char* buffer = NULL;
	FILE* pFile = NULL;
	long lSize = 0;
...
bool rgb2rgb::InitView()
{
	...
	char* pszFragShader = LoadShader(std::string("rgb2rgb.frag"));
...
char* rgb2rgb::LoadYUV ( std::string fileName, int *width, int *height )
{
	long lSize;
	char *buffer = NULL;
	FILE *f;
...
GLuint rgb2rgb::LoadTexture ( std::string fileName )
{
	GLuint texId;
	int width, height;
...
GLuint rgb2rgb::DequeueVideo( void )
{
	GLuint texId;
	int ret;
	...
	if( gCount == 50 )
	{
		...
		f = fopen( "canvas.raw", "w" );
		
		if( f )
		{
			printf("%s: WP %d\n", __FUNCTION__, buf.sequence );
			fwrite( buffer, 1, 640*480*2, f);
			fflush( f );
			fclose( f );
		}
	}
...
******************************************************************************/
bool rgb2rgb::RenderScene()
{
#if 1
	GLuint baseMapTexId = LoadTexture ( std::string("rgb32_640x480.raw") );
#else
...
//Lynx
void rgb2rgb::InitV4L( void )
{
        video_open(&Device, "/dev/video7", 0);
